...
function Run() {
  ...
  this.clients.io = redis.createClient(this.opts.redis.port, this.opts.redis.host);

  this.clients.io.on('error', function(err){
    console.log(err, 'run.js io client');
  });

  this.clients.node = redis.createClient(this.opts.redis.port, this.opts.redis.host);
  ...
  this.statsKey = this.opts.redis.prefix + ':stats:' + this.node.id;
  ...
  node.on('message', function(data){
    ...
    if (!outlet) {
      ...
      self.clients.io.incr(self.statsKey + ':output');
      return;
    }
    ...
    if (self.node.outputs.hasOwnProperty(outlet)) {
      ...
      self.clients.io.incr(self.statsKey + ':outputs:' + outlet);
    }
  });
  ...
  // reset the nodes's stats
  this.clients.io.set(this.statsKey + ':input', 0);
  this.clients.io.set(this.statsKey + ':output', 0);

  _.each(this.node.outputs, function(x, id){
    self.clients.io.set(self.statsKey + ':outputs:' + id, 0);
  });
  ...
  this.brpop = function() {
    ...
    self.clients.io.brpop(self.lkeys, function(err, reply){
      ...
      if(!err){
        ...
        self.clients.io.incr(self.statsKey + ':input');
