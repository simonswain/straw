...
function Runner (def, opts, done) {
  ...
  this.start = function(done) {
    ...
    console.log(pad.timestamp(), 'STARTING', trimPrefix(def.key));
  ...
  this.onChildExit = function(code) {
    if ( self.debug ) {
      console.log(
        pad.timestamp(),
        'EXITED  ',
        pad.space(trimPrefix(def.key), 20),
        code
      );
    }
  ...
  this.onMessage = function(data){
    ...
    if ( data === 'STARTED' ) {
      console.log(pad.timestamp(),  'STARTED ', child.pid, trimPrefix(def.key));
    ...
    console.log(pad.timestamp(), 'MESSAGE ', pad.space(trimPrefix(def.key), 20), data);
    self.emit('message', data);
  };

  this.onDisconnect = function() {
    console.log(pad.timestamp(),  'STOPPED ', trimPrefix(def.key));
    if ( self.stoppedCallback) {
      return self.stoppedCallback();
    }

    if ( self.restarting ) {
      console.log(pad.timestamp(),  'RESTART ', trimPrefix(def.key));
      self.start();
