...
Topology.prototype.create = function(topo, opts, done) {
  ...
  var pkey, prefixedKey, tkey, skey, def, i, x;
  ...
  for ( pkey in topo ) {
    prefixedKey = opts.redis.prefix + ':' + pkey;
    ...
    if ( typeof topo[tkey].node !== 'undefined' ) {
      ...
      if ( typeof def.input !== 'undefined' ) {
        ...
        def.input.forEach(function(x, i) {
          if ( typeof pipes[opts.redis.prefix + ':' + x] !== 'undefined' ) {
            // configured pipe
            def.input[i] = pipes[opts.redis.prefix + ':' + x];
          } else {
            // default pipe (fan out, redis pubsub)
            def.input[i] = {key: opts.redis.prefix + ':' + x};
          }
        });
      }
      ...
      if ( typeof def.inputs !== 'undefined' && Object.prototype.toString.call(def.inputs) === '[object Object]') {
        for(i in def.inputs){
          ...
          if ( typeof pipes[x] !== 'undefined') {
            // configured pipe
            def.inputs[i] = pipes[opts.redis.prefix + ':' + x];
          } else {
            // default pipe (fan out, redis pubsub)
            def.inputs[i] = {key: opts.redis.prefix + ':' + x};
          }
        }
      }
      ...
      if ( typeof def.output !== 'undefined' ) {
        ...
        def.output.forEach(function(x, i) {
          if ( typeof pipes[opts.redis.prefix + ':' + x] !== 'undefined' ) {
            // configured pipe
            def.output[i] = pipes[opts.redis.prefix + ':' + x];
          } else {
            // default pipe (fan out, redis pubsub)
            def.output[i] = {key: opts.redis.prefix + ':' + x};
          }
        });
      }
      ...
      if ( typeof def.outputs !== 'undefined' && Object.prototype.toString.call(def.outputs) === '[object Object]') {
        for(i in def.outputs){
          ...
          if ( typeof pipes[opts.redis.prefix + ':' + x] !== 'undefined') {
            // configured pipe
            def.outputs[i] = pipes[opts.redis.prefix + ':' + x];
          } else {
            // default pipe (fan out, redis pubsub)
            def.outputs[i] = {key: opts.redis.prefix + ':' + x};
          }
        }
      }

      nodes[opts.redis.prefix + ':' + tkey] = topo[tkey];
